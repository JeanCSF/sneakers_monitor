...
async function updateOrCreateSneaker(sneakerObj) {
  ...
  try {
    if (onTest) {
      ...
      console.log(`Sneaker ${sneakerObj.sneakerName} added to test data.`);
    } else {
      const existingSneaker = await SneakerModel.findOne({
        store,
        productReference,
      });
      if (existingSneaker) {
        if (
          ...
          !arraysEqual(
            existingSneaker.availableSizes,
            sneakerObj.availableSizes
          )
        ) {
          existingSneaker.currentPrice = sneakerObj.currentPrice;
          existingSneaker.priceHistory.push({
            price: sneakerObj.currentPrice,
            date: new Date(),
          });
          ...
          console.log(
            "Sneaker already exists in database and price did not change."
          );
        }
      } else {
...
async function processLink(page, link, storeObj) {
  try {
    ...
    await Promise.all([
      ...
      page.waitForSelector(storeObj.selectors.availableSizes),
    ]);
    ...
    if (
      sneakerName.toLowerCase().includes('tênis' || 'tenis') ||
  ...
  try {
    const availableSizes = await page.$$eval(storeObj.selectors.availableSizes, (els) => {
      return els
        ...
        .filter(el => !el.classList.contains('.item_unavaliable'))
